<?php
    session_start();
    include "comun/vars.php";

    if(!isset($_COOKIE['user_id'])){
        header("Location: ". ROOT . "login.php");
    }
    else{
?>
<!DOCTYPE html>
<html lang="es">
    <?php
        include ROOT."comun/head.php";
    ?>
    <body>
        <?php
            include ROOT."comun/nav.php";
        ?>
        <div class="container">
            <div class="row">
                <div class="col-sm-2 col-12"></div>
                <div class="col-sm-8 col-12">
                    <?php
                        include "api/config.php";
                        global $conn;
                        $qry="SELECT *
                                FROM games
                                WHERE type=1 OR type=2 OR type=3
                                ORDER BY date_event ASC";
                        $result=$conn->prepare($qry);
                        $result->bindParam(':user', $_COOKIE['user_id']);
                        $result->execute();

                        $games= $result->fetchAll();

                        function day_of_week($date){
                            switch ($date){
                                case 1: return "Lunes";break;
                                case 2: return "Martes";break;
                                case 3: return "Miércoles";break;
                                case 4: return "Jueves";break;
                                case 5: return "Viernes";break;
                                case 6: return "Sábado";break;
                                case 0: return "Domingo";break;
                                default: return "";
                            } 
                        }
                        $dayWeekAnt="";
                        foreach($games as $game){
                            $dayWeek=day_of_week(date('w', strtotime($game['date_event'])));
                            if($dayWeekAnt!=$dayWeek){
                    ?>
                        <?php if($dayWeekAnt!=""){echo "</div>";} ?>
                        <div class="row">
                            <div class="col-12"><h2 class='title'><?=$dayWeek;?></h2></div>
                    <?php
                            }
                    ?>
                        <div class="col-md-4 col-sm-6 col-12 actividad text-center" actividad="<?=$game['id']?>">
                            <img src="images/<?=$game['image']?>"/>
                            <h3><?=utf8_encode($game['name'])?></h3>
                            <small>Hora: <?=date('H:i', strtotime($game['date_event']));?></small>
                        </div>
                    <?php
                        $dayWeekAnt=$dayWeek;
                        }
                    ?>
                    <table class='table'>
                        <thead>
                            <tr>
                                <th>
                                    <b>HORA</b>
                                </th>
                                <th>
                                    Viernes
                                </th>
                                <th>
                                    Sábado
                                </th>
                                <th>
                                    Domingo
                                </th>
                                <th>
                                    Lunes
                                </th>
                                <th>
                                    Martes
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <th>
                                12:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="#01DFD7">
                                    Hearthstone
                                </td>
                                <td bgcolor="orange">
                                    Buzz
                                </td>
                                <td bgcolor="#01DFD7">
                                    Starcraft
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                13:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="#01DFD7">
                                    Hearthstone
                                </td>
                                <td bgcolor="orange">
                                    Buzz
                                </td>
                                <td bgcolor="#01DFD7">
                                    Starcraft
                                </td>
                                <td bgcolor="orange">
                                    Disfraces
                                </td>
                            </tr>
                            <tr>
                                <th>
                                14:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="#01DFD7">
                                    Hearthstone
                                </td>
                                <td bgcolor="orange">
                                    Buzz
                                </td>
                                <td bgcolor="#01DFD7">
                                    Starcraft
                                </td>
                                <td bgcolor="#01DFD7">
                                    Entrega
                                </td>
                            </tr>
                            <tr>
                                <th>
                                15:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="#01DFD7">
                                    Hearthstone
                                </td>
                                <td bgcolor="orange">
                                    Buzz
                                </td>
                                <td bgcolor="#01DFD7">
                                    Starcraft
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                16:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="#01DFD7">
                                    Hearthstone
                                </td>
                                <td bgcolor="orange">
                                    Buzz
                                </td>
                                <td bgcolor="#01DFD7">
                                    Starcraft
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                17:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                             <tr>
                                <th>
                                18:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                             <tr>
                                <th>
                                19:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Buzz
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Buzz
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                20:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Buzz
                                </td>
                                <td bgcolor="#01DFD7">
                                    Rocket League
                                </td>
                                <td bgcolor="orange">
                                    Extranet + Kona Games
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                21:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="orange">
                                    Buzz + Ultrastar
                                </td>
                                <td bgcolor="#01DFD7">
                                    Rocket League
                                </td>
                                <td bgcolor="#01DFD7">
                                    Trackmania
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                22:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="orange">
                                    Buzz + Ultrastar
                                </td>
                                <td bgcolor="#01DFD7">
                                    Rocket League
                                </td>
                                <td bgcolor="#01DFD7">
                                    Unreal 
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                23:00
                                </th>
                                <td bgcolor="#000000">
                                    
                                </td>
                                <td bgcolor="#01DFD7">
                                    Quake
                                </td>
                                <td bgcolor="orange">
                                    Final Ultrastar y Final Buzz!
                                </td>
                                <td bgcolor="#01DFD7">
                                    Unreal 
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                00:00
                                </th>
                                <td bgcolor="orange">
                                    Minijuegos
                                </td>
                                <td bgcolor="orange">
                                     Minijuegos
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos
                                </td>
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                01:00
                                </th>
                                <td bgcolor="orange">
                                    Minijuegos?
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos?
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos
                                </td>
                                <td bgcolor="orange">
                                     Minijuegos
                                </td>
                                <td bgcolor="#000000">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                02:00
                                </th>
                                <td bgcolor="orange">
                                    Minijuegos?
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos?
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos
                                </td>
                                <td bgcolor="orange">
                                     Minijuegos
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                03:00
                                </th>
                                <td bgcolor="orange">
                                    Minijuegos?
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos?
                                </td>
                                <td bgcolor="orange">
                                    Minijuegos
                                </td>
                                <td bgcolor="orange">
                                     Minijuegos
                                </td>
                                <td bgcolor="black">
                                    
                                </td>
                            </tr>
                            <tr>
                                <th>
                                04:00
                                </th>
                                <td>
                                    
                                </td>
                                <td>
                                    
                                </td>
                                <td>
                                    
                                </td>
                                <td>
                                     
                                </td>
                                <td>
                                    
                                </td>
                            </tr>
                        </tbody>
                </div></div>
            </div>
        </div>
    </body>
    <?php
        include ROOT."comun/libs.php";
    ?>
</html>

<?php } ?>